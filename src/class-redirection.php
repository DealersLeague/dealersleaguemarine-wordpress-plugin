...
/**
 * Class Redirection
 * @package dealersleague\marine\wordpress
 *
 * Thanks to: Custom Permalinks plugin
 */
class Redirection {
	...
	public function custom_post_link( $permalink, $post ) {
		$custom_permalink = get_post_meta( $post->ID, 'listing_permalink', true );
		if ( $custom_permalink ) {
			$post_type = 'post';
	...
	public function custom_page_link( $permalink, $page ) {
		$custom_permalink = get_post_meta( $page, 'listing_permalink', true );
		if ( $custom_permalink ) {
			$language_code = apply_filters( 'wpml_element_language_code', null,
			                                array(
				                                'element_id'   => $page,

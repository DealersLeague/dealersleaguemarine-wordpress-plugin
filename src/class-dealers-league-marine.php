...
class Dealers_League_Marine {
	...
	*/
	...
	public function refresh_listings() {
		...
		/*
		 * Boat Type, Condition, Location, VAT Status, Sale Status, Model, LOA, Draft, Beam, Weight, Manufacturer, price, currency
		 */
		...
		if ( is_array( $listings['listings'] ) && $listings['totalCount'] > 0 ) {
			...
			foreach ( $listings['listings'] as $listing_data ) {
				...
				if ( isset( $json_data['listing']['boat_details']['sales_details']['construction_details']['model'] ) ) {
					$model = $json_data['listing']['boat_details']['sales_details']['construction_details']['model'];
					update_post_meta( $post_id, 'listing_model', $model );
				} else {
					delete_post_meta( $post_id, 'listing_model' );
				}
				if ( isset( $json_data['listing']['boat_details']['sales_details']['construction_details']['manufacturer']['name'][0] ) ) {
					$manufacturer = $json_data['listing']['boat_details']['sales_details']['construction_details']['manufacturer']['name'][0];
